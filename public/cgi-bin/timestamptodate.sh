...
test -z "$tz" && exec cat $HERE/timestamptodate-notz.html

MYTZ=$(httpd -d "$tz") # Asia/Tokyo
RESULT=$(TZ=$MYTZ date -d "@$timestamp" "+%a, %d %b %Y %H:%M:%S %Z")
...
<head>
  ...
  <script src="/css/stopwatch.js"></script>

  <title>Convert timestamp to date and time</title>
  ...
  <div class="container">
    ...
    <div class="info">
      <article class="hero clearfix">
        <div class="col_100">
          <p>Navigation: <a href="/">Clock.Zone</a> / <a href="/tools/">Tools</a> / <a href="/tools/timestamptodate">Timestamp to date</a></p>
</div>
      </article>
      ...
      <article class="article clearfix">
        <div class="col_100">
          <h1>Convert Timestamp to date and time</h1>
          <p>Online tool - Convert Timestamp to date and time for selected timezone. </p>
          <p>What is timestamp?  As UNIX time, timestamp is the number of seconds since 00:00:00 UTC on January 1, 1970. Often used in programming. This tool I build for myself, but it is online, free for use by anybody. </p>
          <div class="col_50">
          <form name="form1" method="get" action="">
            <p>Timestamp to convert:
            <input type="text" name="timestamp" id="timestamp" value="$timestamp">
            [sec.]</p>
            <p>into What timezone?
              <select name="tz" id="tz">
      <option selected>$MYTZ</option>
...
Result: <strong>$RESULT</strong></div>
        
        
        <div class="col_50">
          <h2>Support the webmaster</h2>

          <p>If you like clock.zone, please, support me. </p>
        </div>
</div>
